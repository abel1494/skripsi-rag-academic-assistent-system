...
function DashboardContent() {
  ...
  const handleDeleteFile = async (fileName: string) => {
    if (!confirm(`Yakin ingin menghapus file "${fileName}"? Semua data terkait file ini akan hilang.`)) return;
  
    try {
      const res = await fetch(
        `https://rag-backend-skripsi.vercel.app/delete-file?file_name=${encodeURIComponent(fileName)}&user_id=${userId}`, 
        { method: "DELETE" }
      );
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className={`${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:relative w-72 h-[calc(100vh-64px)] bg-white border-r border-gray-100 flex flex-col z-20 transition-transform duration-300 shadow-xl md:shadow-none`}>
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-2 custom-scrollbar">
            {uploadedFiles.map(f => (
              <div key={f} className="flex items-center justify-between group p-3 hover:bg-gray-50 rounded-xl border border-transparent transition-all">
                <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                  <input 
                    type="checkbox" 
                    checked={selectedFiles.includes(f)} 
                    onChange={e => e.target.checked ? setSelectedFiles(p => [...p, f]) : setSelectedFiles(p => p.filter(x => x !== f))} 
                ...
                <button
                  ...
                  className="ml-2 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                  title="Hapus File"
                >
